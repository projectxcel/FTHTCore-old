...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Fuckthemheggiescoin database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Fuckthemheggiescoin datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Fuckthemheggiescoin datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Fuckthemheggiescoin 0.7 datadir."
    ;;
  3)
    echo "Detected Fuckthemheggiescoin pre-0.8 datadir."
    ;;
  4)
    echo "Detected Fuckthemheggiescoin 0.8 datadir."
    ;;
esac
